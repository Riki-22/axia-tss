# Phase 2: Redis統合 & データアクセス層 - 詳細設計書

**作成日**: 2025-10-15  
**バージョン**: 1.0  
**ステータス**: 設計確定

---

## 📋 目次

1. [概要](#概要)
2. [アーキテクチャ設計](#アーキテクチャ設計)
3. [TTL設計（NYクローズ基準）](#ttl設計nyクローズ基準)
4. [データフォーマット（MessagePack）](#データフォーマットmessagepack)
5. [実装仕様](#実装仕様)
6. [テスト戦略](#テスト戦略)

---

## 概要

### 目的

**4層キャッシュ戦略によるパフォーマンス向上と可用性確保**

- Redis統合による24時間データキャッシュ
- MT5接続競合の60%削減
- レスポンス時間90%改善（100ms → 10ms）
- システム可用性向上（MT5障害時も継続稼働）

### 主要成果物

```
src/infrastructure/persistence/redis/
├── __init__.py
├── redis_client.py      # 接続管理（シングルトン）
└── price_cache.py       # OHLCVキャッシュロジック

src/domain/repositories/
└── market_data_repository.py  # インターフェース定義
```

---

## アーキテクチャ設計

### レイヤー構成

```
┌─────────────────────────────────────────────────┐
│ Domain Layer                                    │
│  └─ IMarketDataRepository (インターフェース)    │
└─────────────────────────────────────────────────┘
                    ↑ 実装
┌─────────────────────────────────────────────────┐
│ Infrastructure Layer                            │
│  ├─ RedisClient (接続管理)                      │
│  └─ PriceCache (キャッシュロジック)             │
└─────────────────────────────────────────────────┘
                    ↓ 利用
┌─────────────────────────────────────────────────┐
│ ElastiCache for Redis                           │
│  └─ t4g.micro (512MB) @ $12.67/月               │
└─────────────────────────────────────────────────┘
```

### 責務分離

#### RedisClient（接続管理）

**役割**: 低レベルなRedis操作の提供

```python
責務:
├─ シングルトンパターンで接続を1つだけ維持
├─ 接続プール管理（max_connections=10）
├─ リトライロジック（最大3回、指数バックオフ）
├─ ヘルスチェック機能
└─ 基本操作（get/set/delete/exists/keys）

提供メソッド:
├─ get_instance() → RedisClient
├─ ping() → bool
├─ get(key) → Optional[bytes]
├─ set(key, value, ex) → bool
├─ delete(key) → bool
├─ exists(key) → bool
├─ keys(pattern) → List[str]
└─ close() → None
```

#### PriceCache（キャッシュロジック）

**役割**: OHLCVデータ専用の高レベル操作

```python
責務:
├─ NYクローズ基準のTTL管理
├─ MessagePackによるシリアライズ
├─ キーフォーマット統一
├─ メモリ使用量監視
└─ キャッシュ統計情報

提供メソッド:
├─ save_ohlcv(df, symbol, timeframe) → bool
├─ load_ohlcv(symbol, timeframe, start_date, end_date, days) → Optional[DataFrame]
├─ exists(symbol, timeframe, date) → bool
├─ get_available_range(symbol, timeframe) → Optional[Tuple[datetime, datetime]]
├─ delete(symbol, timeframe) → bool
├─ clear_symbol(symbol) → int
├─ get_memory_usage() → Dict
└─ get_cache_stats() → Dict
```

---

## TTL設計（NYクローズ基準）

### 設計方針

**採用方針**: 米国市場クローズ（NYクローズ）を基準にデータを管理

#### 理由

```
✓ FX市場の取引サイクルと整合
✓ 「今日のデータ」が明確に定義できる
✓ 週末の無駄なメモリ使用を回避
✓ データ更新タイミングが自然
```

### FX市場タイムライン

```
NYクローズ時刻（日本時間）:
├─ 夏時間（3月第2日曜 - 11月第1日曜）: 翌朝 06:00 JST
└─ 冬時間（11月第1日曜 - 3月第2日曜）: 翌朝 07:00 JST

1週間の流れ:
日曜 22:00 JST   市場オープン（ウェリントン）
月曜 06:00 JST   NYクローズ（前週金曜分）★データ削除
火曜 06:00 JST   NYクローズ（月曜分）★データ削除
水曜 06:00 JST   NYクローズ（火曜分）★データ削除
木曜 06:00 JST   NYクローズ（水曜分）★データ削除
金曜 06:00 JST   NYクローズ（木曜分）★データ削除
土曜 06:00 JST   NYクローズ（金曜分）
土曜 07:00 JST   市場完全クローズ
  ↓ 週末保持（約40時間）
日曜 22:00 JST   市場再オープン
月曜 06:00 JST   ★週末データ削除
```

### TTL計算ロジック

```python
def _calculate_ttl_until_ny_close(current_time: datetime) -> int:
    """
    現在時刻から次のNYクローズまでの秒数を計算
    
    例:
    - 火曜 10:00 JST → 水曜 06:00 JST = 20時間 = 72,000秒
    - 火曜 05:00 JST → 火曜 06:00 JST = 1時間 = 3,600秒
    - 金曜 20:00 JST → 土曜 06:00 JST = 10時間 = 36,000秒
    
    週末:
    - 土曜 10:00 JST → 月曜 06:00 JST = 約44時間
    """
    
    # 1. 夏時間判定
    is_dst = _is_dst(current_time)
    ny_close_hour = 6 if is_dst else 7
    
    # 2. 次のNYクローズ時刻を計算
    next_close = current_time.replace(
        hour=ny_close_hour, minute=0, second=0, microsecond=0
    )
    
    # 3. 既に過ぎている場合は翌日
    if current_time >= next_close:
        next_close += timedelta(days=1)
    
    # 4. 週末処理：土曜の場合は月曜まで保持
    if next_close.weekday() == 5:  # 土曜
        next_close += timedelta(days=2)  # 月曜まで
    elif next_close.weekday() == 6:  # 日曜
        next_close += timedelta(days=1)  # 月曜まで
    
    # 5. TTL秒数を計算
    ttl_seconds = int((next_close - current_time).total_seconds())
    
    return max(ttl_seconds, 3600)  # 最低1時間は保持
```

### 週末データの扱い

**採用方針**: 週末データは月曜オープンまで保持（約40時間）

```
土曜 06:00 JST クローズ
    ↓ 保持期間（約40時間）
月曜 06:00 JST 削除

理由:
✓ 週末に分析作業をする可能性を考慮
✓ データの連続性を確保
✓ メモリ使用量は許容範囲内（50MB以下）
```

### キャッシュ更新戦略

**採用方針**: リアルタイム更新 + 定期クリーンアップ

```
リアルタイム更新:
├─ データ取得の都度、Redisに保存
├─ TTLは次のNYクローズまで自動計算
└─ 鮮度の高いデータを常に提供

定期クリーンアップ:
├─ 毎朝 08:00 JST に実行
├─ 期限切れキーを明示的に削除
└─ メモリ使用量を監視・アラート
```

---

## データフォーマット（MessagePack）

### 採用理由

**採用方針**: MessagePackを使用してDataFrameをシリアライズ

#### pickleと比較した優位性

| 項目 | pickle | MessagePack | 判定 |
|------|--------|-------------|------|
| **速度** | 高速 | 非常に高速 | ✓ |
| **サイズ** | 中程度 | 小さい | ✓ |
| **言語間互換性** | Python専用 | 多言語対応 | ✓ |
| **セキュリティ** | 低（任意コード実行リスク） | 高（データのみ） | ✓ |
| **デバッグ** | バイナリ（不可読） | バイナリ（不可読） | - |
| **pandas互換性** | ネイティブ | ライブラリ経由 | ✓ |

#### 具体的なメリット

```python
1. パフォーマンス
   - シリアライズ: pickle比で約20%高速
   - デシリアライズ: pickle比で約15%高速
   - データサイズ: pickle比で約10-20%削減

2. セキュリティ
   - pickleは任意のPythonオブジェクトを復元可能
   - MessagePackはデータ構造のみ（安全）

3. 将来の拡張性
   - 他言語（Go, Rustなど）からもアクセス可能
   - マイクロサービス化に対応

4. 標準化
   - 業界標準のシリアライゼーションフォーマット
   - 多くのツールで対応
```

### 実装方法

```python
import msgpack
import pandas as pd
import numpy as np

def serialize_dataframe(df: pd.DataFrame) -> bytes:
    """
    DataFrameをMessagePack形式にシリアライズ
    
    Args:
        df: OHLCV DataFrame
            - index: DatetimeIndex (UTC)
            - columns: ['open', 'high', 'low', 'close', 'volume']
    
    Returns:
        bytes: MessagePack形式のバイナリデータ
    
    Note:
        - indexのdatetimeをUNIXタイムスタンプ(秒)に変換
        - 列指向の辞書形式で保存（圧縮効率が良い）
    """
    df_copy = df.copy()
    
    # DatetimeIndexをUNIXタイムスタンプ（秒単位）に変換
    if isinstance(df_copy.index, pd.DatetimeIndex):
        df_copy.index = df_copy.index.astype(np.int64) // 10**9
    
    # 列指向の辞書に変換（'time'列を含む）
    data_to_pack = df_copy.reset_index().to_dict('list')
    
    return msgpack.packb(data_to_pack, use_bin_type=True)


def deserialize_dataframe(data: bytes) -> pd.DataFrame:
    """
    MessagePack形式からDataFrameを復元
    
    Args:
        data: MessagePack形式のバイナリデータ
    
    Returns:
        pd.DataFrame: OHLCV DataFrame
            - index: DatetimeIndex (UTC)
            - columns: ['open', 'high', 'low', 'close', 'volume']
    
    Raises:
        ValueError: データが不正な形式の場合
    """
    try:
        unpacked_data = msgpack.unpackb(data, raw=False)
        
        # DataFrameに復元
        df = pd.DataFrame(unpacked_data)
        
        # 'time'列または'index'列をインデックスに設定
        time_col = 'time' if 'time' in df.columns else 'index'
        df = df.set_index(time_col)
        
        # UNIXタイムスタンプからdatetimeに復元（UTC）
        df.index = pd.to_datetime(df.index, unit='s', utc=True)
        df.index.name = 'time'
        
        # 数値型を明示的に設定（念のため）
        for col in ['open', 'high', 'low', 'close']:
            if col in df.columns:
                df[col] = df[col].astype(np.float64)
        if 'volume' in df.columns:
            df[col] = df['volume'].astype(np.int64)
        
        return df
        
    except Exception as e:
        raise ValueError(f"Failed to deserialize DataFrame: {e}")

### メモリ効率

```
1通貨ペア × 1タイムフレーム × 24時間（H1）の場合:

DataFrame (メモリ上):
  24本 × 6カラム × 8バイト（float64） = 約1.2KB

MessagePack (Redis):
  メタデータ + データ = 約0.8KB（約33%削減）

3通貨ペア × 3タイムフレーム × 1,440本（M1）:
  約1.3MB（メモリ上）
  約0.9MB（MessagePack）

目標: 50MB以内
実測見込み: 約30-35MB
```

---

## 実装仕様

### キーフォーマット

**採用方針**: 日付を含まない単一キーフォーマット

```
フォーマット:
  ohlcv:{symbol}:{timeframe}

例:
  ohlcv:USDJPY:H1
  ohlcv:EURUSD:M15
  ohlcv:GBPJPY:D1

理由:
✓ シンプルで理解しやすい
✓ 1通貨ペア×1タイムフレームは常に最新24時間分
✓ TTLで自動削除されるため複数日管理不要
✓ パターンマッチで柔軟な操作が可能

パターンマッチ例:
  keys("ohlcv:USDJPY:*")  → USDJPYの全タイムフレーム
  keys("ohlcv:*:H1")      → 全通貨ペアのH1
  keys("ohlcv:*")         → 全OHLCVデータ
```

### メタデータ管理

```python
各キャッシュに付随するメタデータ（別キーで保存）:

ohlcv:USDJPY:H1          ← 実データ（MessagePack）
ohlcv:USDJPY:H1:meta     ← メタデータ（JSON）
  ├─ trading_day: "20251015"
  ├─ cached_at: "2025-10-15T10:30:00Z"
  ├─ data_start: "2025-10-14T10:00:00Z"
  ├─ data_end: "2025-10-15T10:00:00Z"
  ├─ row_count: 24
  └─ ttl_expires_at: "2025-10-16T06:00:00Z"
```

### エラーハンドリング

```python
エラー種別と対応:

1. ConnectionError（接続エラー）
   - リトライ3回（指数バックオフ: 1秒, 2秒, 4秒）
   - 失敗時はログ記録してNoneを返す
   - 上位層でフォールバック（MT5 → S3 → yfinance）

2. TimeoutError（タイムアウト）
   - socket_timeout=5秒
   - リトライなしでNoneを返す
   - 上位層でフォールバック

3. DataError（データ破損）
   - MessagePack unpack失敗
   - 該当キーを削除
   - ログにERROR記録
   - Noneを返す

4. MemoryError（メモリ不足）
   - 古いキャッシュを削除（LRU的に）
   - CloudWatchアラート送信
   - 処理を継続（致命的エラーとしない）
```

### 監視・統計情報

```python
get_cache_stats() が返す情報:

{
    "total_keys": 15,
    "symbols": ["USDJPY", "EURUSD", "GBPJPY"],
    "timeframes": ["H1", "M15", "D1"],
    "memory_used_mb": 35.2,
    "memory_limit_mb": 50.0,
    "memory_usage_pct": 70.4,
    "oldest_data": "2025-10-14T06:00:00Z",
    "newest_data": "2025-10-15T10:30:00Z",
    "cache_hit_rate": 0.65,  # 65%
    "total_requests": 1000,
    "cache_hits": 650,
    "cache_misses": 350
}
```

---

## テスト戦略

### 単体テスト（ローカル環境）

```python
# test_redis_client.py
def test_singleton_pattern():
    """シングルトンパターンのテスト"""
    client1 = RedisClient.get_instance()
    client2 = RedisClient.get_instance()
    assert client1 is client2

def test_connection_retry():
    """リトライロジックのテスト"""
    # Redisを一時停止してテスト
    pass

def test_basic_operations():
    """基本操作のテスト"""
    client = RedisClient.get_instance()
    assert client.set("test_key", b"test_value", ex=60)
    assert client.get("test_key") == b"test_value"
    assert client.exists("test_key")
    assert client.delete("test_key")
    assert not client.exists("test_key")
```

```python
# test_price_cache.py
def test_save_load_ohlcv():
    """OHLCV保存・読み込みのテスト"""
    cache = PriceCache()
    df = create_test_dataframe()
    
    assert cache.save_ohlcv(df, "USDJPY", "H1")
    loaded = cache.load_ohlcv("USDJPY", "H1")
    
    assert loaded is not None
    assert len(loaded) == len(df)
    pd.testing.assert_frame_equal(loaded, df)

def test_ttl_calculation():
    """TTL計算のテスト"""
    # 火曜 10:00 JST の場合
    current = datetime(2025, 10, 14, 10, 0, 0)
    ttl = cache._calculate_ttl_until_ny_close(current)
    assert 19 * 3600 < ttl < 21 * 3600  # 約20時間

def test_dst_detection():
    """夏時間判定のテスト"""
    summer = datetime(2025, 7, 1)
    winter = datetime(2025, 1, 1)
    assert cache._is_dst(summer) == True
    assert cache._is_dst(winter) == False

def test_weekend_handling():
    """週末データのテスト"""
    # 金曜 20:00 JST → 月曜 06:00 JST
    friday = datetime(2025, 10, 17, 20, 0, 0)
    ttl = cache._calculate_ttl_until_ny_close(friday)
    assert 58 * 3600 < ttl < 62 * 3600  # 約60時間
```

### 統合テスト（EC2環境）

```python
def test_full_integration():
    """エンドツーエンドのテスト"""
    # 1. MT5からデータ取得
    df = mt5_collector.fetch_ohlcv_data("USDJPY", "H1", 24)
    
    # 2. Redisに保存
    cache = PriceCache()
    assert cache.save_ohlcv(df, "USDJPY", "H1")
    
    # 3. Redisから読み込み
    cached = cache.load_ohlcv("USDJPY", "H1")
    assert cached is not None
    
    # 4. データの整合性確認
    pd.testing.assert_frame_equal(df, cached)
    
    # 5. TTL確認
    ttl = cache.redis_client.ttl("ohlcv:USDJPY:H1")
    assert ttl > 0

def test_performance_benchmark():
    """パフォーマンステスト"""
    cache = PriceCache()
    
    # 保存速度測定
    start = time.time()
    for i in range(100):
        cache.save_ohlcv(test_df, "USDJPY", "H1")
    save_time = time.time() - start
    assert save_time < 1.0  # 100回で1秒以内
    
    # 読み込み速度測定
    start = time.time()
    for i in range(100):
        cache.load_ohlcv("USDJPY", "H1")
    load_time = time.time() - start
    assert load_time < 0.5  # 100回で0.5秒以内
```

### パフォーマンス目標

| 指標 | 目標値 | 測定方法 |
|------|--------|----------|
| **保存速度** | < 10ms | 1回の save_ohlcv() |
| **読み込み速度** | < 5ms | 1回の load_ohlcv() |
| **メモリ使用量** | < 50MB | get_memory_usage() |
| **キャッシュヒット率** | > 60% | get_cache_stats() |
| **TTL精度** | ±1分以内 | NYクローズ時刻との誤差 |

---

## 実装チェックリスト

### Week 1: Redis基盤実装

- [ ] `redis_client.py` 実装
  - [ ] シングルトンパターン
  - [ ] 接続プール設定
  - [ ] リトライロジック
  - [ ] 基本操作メソッド
  - [ ] ヘルスチェック
  - [ ] 単体テスト

- [ ] `price_cache.py` 実装
  - [ ] NYクローズTTL計算
  - [ ] 夏時間判定
  - [ ] MessagePackシリアライズ
  - [ ] save_ohlcv() / load_ohlcv()
  - [ ] メタデータ管理
  - [ ] 統計情報機能
  - [ ] 単体テスト

- [ ] `__init__.py` 作成

### Week 2: 統合テスト

- [ ] EC2環境での動作確認
- [ ] MT5連携テスト
- [ ] パフォーマンステスト
- [ ] メモリ使用量測定
- [ ] TTL動作確認（週末含む）

### Week 3: ドキュメント

- [ ] ADR-006更新
- [ ] README更新
- [ ] 運用手順書作成

---

## 付録

### 設定ファイル（redis_config.py）

```python
# NYクローズ時刻設定
NY_CLOSE_HOUR_EDT = 6  # 夏時間: JST 06:00
NY_CLOSE_HOUR_EST = 7  # 冬時間: JST 07:00

# 週末データ保持
WEEKEND_RETENTION_HOURS = 40

# キャッシュクリーンアップ
CACHE_CLEANUP_HOUR = 8  # 毎朝8時

# MessagePack設定
MSGPACK_USE_BIN_TYPE = True
```

### 依存ライブラリ

```
redis==5.0.0
msgpack==1.0.7
pandas==2.1.0
pytz==2023.3
```

---

**END OF DOCUMENT**